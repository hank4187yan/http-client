...
HTTP_API const char* ft_http_sync_request(ft_http_client_t* http, const char* url, http_request_method_e m, char* body)
{
	if(http == NULL)
	{
		return NULL;
	...
	if (http->method == M_GET) {
		...
		http->error_code = http_internal_sync_request(http, url, body, body_size, 0, 0);
	} else {
